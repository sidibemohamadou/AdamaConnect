...
export function registerAuthRoutes(app: Express) {
  ...
  // Supporter également GET /api/logout pour les boutons qui font une navigation directe
  // Plusieurs pages utilisent `window.location.href = '/api/logout'` (GET). Fournir
  // une réponse HTML qui effectue une redirection client-side après destruction de la session.
  app.get("/api/logout", (req, res) => {
    // Si passport est présent, appeler req.logout si disponible
    try {
    ...
    if (!req.session) {
      // Toujours renvoyer une page qui redirige à la racine
      res.send(`
        <!DOCTYPE html>
        <html>
        <head>
